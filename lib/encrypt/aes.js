...
var AES_Sbox_Inv = undefined
var AES_ShiftRowTab_Inv = undefined
var AES_xtime = undefined
...
function encryptLongString(str, key) {
  var data = '';
  for (var i = 0; i < str.length; i = i + 16) {
    data += encrypt(str.substr(i, 16), key);
  }
  return data;
}

function decryptLongString(str, key) {
  var data = '';
  for (var i = 0; i < str.length; i = i + 16) {
    data += decrypt(str.substr(i, 16), key);
  }
  return data;
}
...
export const aes = {
  ...
  encode: (str, key) => {
    ...
    AES_Init()
    var bin_key = string2Bin(key)
    AES_ExpandKey(bin_key)

    const final_encoded = str.length > 16 ?
      encryptLongString(str, bin_key) :
      encrypt(str, bin_key)
  ...
  decode: (str, key) => {
    ...
    const final_decoded = str.length > 16 ?
      decryptLongString(str, bin_key) :
      decrypt(str, bin_key)
