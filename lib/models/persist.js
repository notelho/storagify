...
export const persist = new class {
    ...
    get(key, storageInstance) {
        ...
        if (
            !key || (
                ...
                !storageInstance.getItem(encoder._force_hash(key))
            )
        ) return undefined
        ...
        if (this.dev)
            ...
            return JSON.parse(encoder.decode(v))
    ...
    _auto_switch(key, storageInstance) {
        ...
        if (typeof prod_value === 'string') {
            ...
            try {
                ...
                if (value === encoder._force_decode(prod_value))
                    v_def = false
        ...
        if (v_def) {
            ...
            if (v_dev && !this.dev)
                this._dev_to_prod(key, value, storageInstance)

            else if (v_prod && this.dev)
                this._prod_to_dev(key, value, storageInstance)
    ...
    _prod_to_dev(key, value, storageInstance) {
        ...
        try {
            ...
            when = encoder.when(enc_value).getTime()
